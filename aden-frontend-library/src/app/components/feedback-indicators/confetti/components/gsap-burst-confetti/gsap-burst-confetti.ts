...
@Component({
  selector: 'app-gsap-burst-confetti',
  imports: [UiPlayground],
  templateUrl: './gsap-burst-confetti.html',
  styleUrl: './gsap-burst-confetti.scss',
})
export class GsapBurstConfetti extends BaseUi {
  override meta: ComponentMeta = {
    title: 'GSAP Burst Confetti',
    description: 'A DOM-based particle explosion using GreenSock (GSAP) for smooth physics animations.',
    slug: 'gsap-burst-confetti',
  };
  ...
  private animateParticle(particle: HTMLElement) {
    ...
    tl.to(particle, {
      y: `+=${Math.random() * (this.particleGravityMax - this.particleGravity) + this.particleGravity}`,
      opacity: 0,
